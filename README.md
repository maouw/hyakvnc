...
- A HyakVNC-compatible Apptainer container image in a directory on Hyak or the URL to one (e.g,., `oras://ghcr.io/maouw/ubuntu22.04_turbovnc:latest`)
...
- `oras://ghcr.io/maouw/ubuntu22.04_turbovnc:latest` -- Ubuntu 22.04 with TurboVNC
...
```bash
tf=$(mktemp) && curl -o "$tf" -fsSL https://raw.githubusercontent.com/maouw/hyakvnc/shell-turbovnc/hyakvnc && bash "$tf" install && rm -f "$tf" && unset tf && [[ ":${PATH}:" != *":$HOME/.local/bin:"* ]] && export PATH="$HOME/.local/bin:$PATH" && [-n "${ZSH_VERSION:-}" ] && rehash
```
