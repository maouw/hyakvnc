...
```bash
tf=$(mktemp) && curl -o "$tf" -fsSL https://raw.githubusercontent.com/maouw/hyakvnc/shell-turbovnc/hyakvnc && bash "$tf" install && rm "$mm" && unset tf && [[ ":${PATH}:" != *":$HOME/.local/bin:"* ]] && export PATH="$HOME/.local/bin:$PATH" && [-n "${ZSH_VERSION:-}" ] && rehash
```
