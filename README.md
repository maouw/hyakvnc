...
```bash
tf=$(mktemp) && curl -o "$tf" -fsSL https://raw.githubusercontent.com/maouw/hyakvnc/main/hyakvnc && bash "$tf" install && rm -f "$tf" && unset tf && [[ ":${PATH}:" != *":$HOME/.local/bin:"* ]] && export PATH="$HOME/.local/bin:$PATH" && [-n "${ZSH_VERSION:-}" ] && rehash
```
