...
class HyakVncInstance:
    def __init__(
        ...
        try:
            name_meta = re.match(
                rf"(?P<prefix>{instance_prefix})-(?P<jobid>\d+)-(?P<appinstance>.*)",
                self.apptainer_instance_info.instance_name,
            ).groupdict()
    ...
    def is_alive(self):
        return self.vnc_pid_file_exists() and check_remote_pid_exists_and_port_open(
            self.compute_node, self.apptainer_instance_info.pid, self.vnc_port
        )
    ...
    def load_instance(
        ...
        read_apptainer_config: Optional[bool] = False,
        apptainer_config_dir: Optional[Union[str, Path]] = None,
    ) -> Union["HyakVncInstance", None]:
        ...
        if instance_name:
            ...
            path = (
                Path(apptainer_config_dir).expanduser() / "instances" / "app" / instance_name / f"{instance_name}.json"
            )
        else:
        ...
        hyakvnc_instance = HyakVncInstance(
            apptainer_instance_info=apptainer_instance_info,
    ...
    def find_running_instances(
        ...
        for p in vnc_instance_files:
            ...
            instance = HyakVncInstance(
                instance_info, instance_prefix=instance_prefix, apptainer_config_dir=apptainer_config_dir
            )
