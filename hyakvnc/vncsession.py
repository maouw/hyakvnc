...
class ManualConnectionStringGenerator(ConnectionStringGenerator):
    ...
    def __str__(self):
        out = f"Configure your SSH client to connect to the *address* '{self.compute_node}' through a *jump host* as '{self.login_node}' with local port forwarding from {self.port_on_client} on your machine ('localhost' or 127.0.0.1) to the port {self.port_on_compute_node} on the remote host. In your VNC client, connect to 'localhost:{self.port_on_client}'."
        return out
